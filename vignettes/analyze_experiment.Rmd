...
if (!exists("estimates"))  { 
  calculate<-TRUE
} else if (length(estimates)==0) {
  calculate<-TRUE
}

df<- data %>% dplyr::filter(subject<="AK")
...
Plot all data (actually a subset so that vignette execution doesn't take too long). 
...
```{r, echo=FALSE, message=TRUE, fig.height=36, fig.width=10, fig.cap = "First column is left stream. Second column is right stream. Each subject gets a pair of rows, "1" for upright letter trials, "2" for inverted letter trials."}
