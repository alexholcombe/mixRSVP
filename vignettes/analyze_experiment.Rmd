...
dataAAinvertedLeft <- data %>% dplyr::filter(subject=="AA",orientation=="Inverted",stream=="Left")
analyzeOneCondition(dataSmall,numItemsInStream,parameterBounds(), nReplicates=3)
...
Plot all data (actually a subset do vignette execution doesn't take too long). First column is left stream. Second column is right stream. Each subject gets a pair of rows, "1" for upright letter trials, "2" for inverted letter trials.
