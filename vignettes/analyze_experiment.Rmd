...
```{r plot one subject, echo=FALSE, message=FALSE, fig.cap = "Fitted guessing distribution in yellow, discretized Gaussian in light blue, combined in green, dark blue is continuous Gaussian"}
...
```{r calculate curves, echo=FALSE, message=TRUE, fig.height=18, fig.width=10}
...
```{r Scale fine-grained Gaussian, echo=FALSE, message=TRUE}
...
Plot everything 
```{r plot everything, echo=FALSE, message=TRUE, fig.height=26, fig.width=10, fig.cap = "Left target (first column) and right target (second column). Each subject gets two rows, one for upright letter trials, one for inverted letter trials."}
